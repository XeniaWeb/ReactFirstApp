...
interface CreateFabricProps {
    onCreate: (fabric: IFabric) => void;
}

export default function CreateFabric({onCreate}: CreateFabricProps) {
    ...
    const submitHandler = async (event: React.FormEvent) => {
        ...
        fabricData.price = Number(fabricPrice);
        fabricData.description = fabricDesc;

        const response = await axios.post<IFabric>('https://fakestoreapi.com/products', fabricData)
    ...
    const changePriceHandler = (event: ChangeEvent<HTMLInputElement>) => {
        setErrorPrice('');
        setFabricPrice(event.target.value);
    };
    ...
    return (
        <form onSubmit={submitHandler}>
            ...
            <div className="md:flex justify-between items-center mb-2">
                <label htmlFor="fabric-price" className="px-1 text-lg"> Fabric&nbsp;Price: </label>
                <input id="fabric-price" type="text"
                       className="border-2 rounded w-full md:w-3/4 p-2 outline-emerald-700/50"
